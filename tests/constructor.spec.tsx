...
test.describe('Страница конструктора бургера (Интеграционные тесты)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    // ТРЕБОВАНИЕ 1: Настроен перехват всех запросов к бэкенду с помощью page.routeFromHAR
    // HAR-файл будет автоматически создан при первом запуске или использоваться для мокинга
    await page.routeFromHAR('tests/har/ingredients.har', {
      url: '**/api/ingredients',
      update: false // Меняйте на true, если нужно перезаписать реальные запросы в HAR
    });
  ...
  test.describe('Создание заказа с авторизацией', () => {
    ...
    test.beforeEach(async ({ context, page }) => {
      // ТРЕБОВАНИЕ 2: Перед выполнением теста создания заказа подставляются фейковые токены
    ...
    // ТРЕБОВАНИЕ 2: После завершения теста токены обязательно очищаются
